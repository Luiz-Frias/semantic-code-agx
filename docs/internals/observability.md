...
### Tracing Filter Controls (Infrastructure Spans)

Infrastructure-level tracing (span tree + structured events) is emitted as JSON
to stderr by the CLI runtime.

- `RUST_LOG` controls tracing filters and takes precedence when set.
- `--log-level` is a CLI fallback when `RUST_LOG` is unset.
  Accepted values: `error`, `warn`, `info`, `debug`, `trace` (default: `info`).

Examples:

```bash
RUST_LOG=semantic_code_adapters=trace,semantic_code_app=info sca search --query "auth"
```

```bash
sca --log-level debug search --query "auth"
```
