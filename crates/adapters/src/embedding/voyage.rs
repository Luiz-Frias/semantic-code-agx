...
/// Voyage embedding adapter configuration.
#[derive(Debug, Clone)]
pub struct VoyageEmbeddingConfig {
    /// API key used for authentication.
    pub api_key: Box<str>,
    /// Embedding model name (defaults to `voyage-4`).
    pub model: Option<Box<str>>,
    /// Base URL override (defaults to `https://api.voyageai.com/v1`).
...
impl VoyageEmbedding {
    /// Create a new Voyage embedding adapter.
    pub fn new(config: &VoyageEmbeddingConfig) -> Result<Self> {
        ...
        let client = reqwest::Client::builder()
            ...
            .map_err(|error| {
                ErrorEnvelope::unexpected(
                    ErrorCode::new("embedding", "voyage_client_init_failed"),
                    format!("failed to build Voyage client: {error}"),
                    ErrorClass::NonRetriable,
                )
            })?;

        let provider = EmbeddingProviderInfo {
            id: EmbeddingProviderId::parse("voyage").map_err(ErrorEnvelope::from)?,
            name: "Voyage".into(),
        };
    ...
    async fn embed_many(
        ...
        let request = VoyageEmbeddingRequest {
            ...
            input: if expected_count == 1 {
                ...
                VoyageInput::Single(text)
            } else {
                VoyageInput::Many(texts)
            },
            input_type: None,
        ...
        map_voyage_embeddings(response, expected_count, self.dimension_override)
    }
    ...
    async fn send_request(
        ...
        operation: &'static str,
    ) -> Result<VoyageEmbeddingResponse> {
        ctx.ensure_not_cancelled(operation)?;
        ...
        if !status.is_success() {
            return Err(map_voyage_http_error(status, &payload));
        }

        serde_json::from_slice(&payload).map_err(|error| {
            ErrorEnvelope::unexpected(
                ErrorCode::new("embedding", "voyage_invalid_response"),
...
impl EmbeddingPort for VoyageEmbedding {
    ...
    fn detect_dimension(
        ...
        Box::pin(async move {
            ctx.ensure_not_cancelled("voyage_embedding.detect_dimension")?;
            if let Some(dimension) = dimension_override {
                return Ok(dimension);
            }
            let vectors = self
                .embed_many(&ctx, vec![test_text], "voyage_embedding.detect_dimension")
                .await?;
    ...
    fn embed(
        ...
        Box::pin(async move {
            let mut vectors = self
                .embed_many(&ctx, vec![text], "voyage_embedding.embed")
                .await?;
    ...
    fn embed_batch(
        ...
        let span = tracing::info_span!(
            "adapter.embedding.voyage.embed_batch",
            provider = "voyage",
            batch_size
        );
        Box::pin(
            async move {
                self.embed_many(&ctx, texts, "voyage_embedding.embed_batch")
                    .await
            }
            .instrument(span),
        )
...
struct VoyageEmbeddingResponse {
    ...
    #[serde(default)]
    data: Option<Vec<VoyageEmbeddingDatum>>,
}

#[derive(Debug, Deserialize)]
struct VoyageEmbeddingDatum {
    embedding: Vec<f32>,
    index: Option<usize>,
}
...
struct VoyageErrorResponse {
    ...
    error: Option<VoyageErrorDetail>,
}
...
fn map_reqwest_error(error: &reqwest::Error) -> ErrorEnvelope {
    ...
    if error.is_connect() {
        return ErrorEnvelope::unexpected(
            ErrorCode::io(),
            format!("Voyage connection failed: {error}"),
            ErrorClass::Retriable,
        );
    }
    ErrorEnvelope::unexpected(
        ErrorCode::new("embedding", "voyage_request_failed"),
        format!("Voyage request failed: {error}"),
        ErrorClass::NonRetriable,
    )
}

fn map_voyage_http_error(status: StatusCode, payload: &[u8]) -> ErrorEnvelope {
    let message = serde_json::from_slice::<VoyageErrorResponse>(payload)
        .ok()
        .and_then(|response| {
            response
                .error
        ...
        .unwrap_or_else(|| "Voyage request failed".to_string());
    ...
    let mut envelope = match status.as_u16() {
        ...
        _ => ErrorEnvelope::unexpected(
            ErrorCode::new("embedding", "voyage_http_error"),
            message,
            ErrorClass::NonRetriable,
        ),
    };
...
fn map_voyage_embeddings(
    ...
    let all_indexed = data.iter().all(|datum| datum.index.is_some());
    if all_indexed {
        let mut slots: Vec<Option<EmbeddingVector>> = vec![None; expected_count];
        for datum in data {
            let index = datum.index.ok_or_else(|| {
                ErrorEnvelope::unexpected(
                    ErrorCode::internal(),
                    "embedding response index missing",
                    ErrorClass::NonRetriable,
                )
            })?;
            ...
            *slot = Some(map_embedding_values(datum.embedding, expected_dimension)?);
        }
        return slots
            .into_iter()
            .map(|slot| {
    ...
    data.into_iter()
        .map(|datum| map_embedding_values(datum.embedding, expected_dimension))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn voyage_request_serializes_batch_input() {
        let request = VoyageEmbeddingRequest {
            model: "voyage-4".into(),
            input: VoyageInput::Many(vec!["a".into(), "b".into()]),
            input_type: None,
    ...
    #[test]
    fn map_voyage_embeddings_rejects_dimension_mismatch() {
        let response = VoyageEmbeddingResponse {
            data: Some(vec![VoyageEmbeddingDatum {
                embedding: vec![0.1, 0.2],
                index: Some(0),
            }]),
            embeddings: None,
        };
        let error = map_voyage_embeddings(response, 1, Some(3)).unwrap_err();
        assert_eq!(error.code, ErrorCode::invalid_input());
    }

    #[test]
    fn map_voyage_http_error_rate_limited_is_retriable() {
        ...
        .unwrap();
        let envelope = map_voyage_http_error(StatusCode::TOO_MANY_REQUESTS, &payload);
